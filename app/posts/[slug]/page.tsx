...
function getOGImage(title: string, category: string, date: string): string {
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: post.coverImage ? [{ url: post.coverImage, alt: title }] : [{ url: getOGImage(title, post.category, post.date), alt: title }],
    },
    twitter: {
      card: "summary_large_image",
      title,
      description,
      images: post.coverImage ? [post.coverImage] : [getOGImage(title, post.category, post.date)],
    },
    other: {
      "json-ld": JSON.stringify({
        ...
        url,
        image: post.coverImage || getOGImage(title, post.category, post.date),
      }),
    },
  };
}
  ...
  return (
    <Container width="narrow">
      <div className="post-page">
        ...
        {/* Cover image — use explicit coverImage or fallback to OG-generated */}
        {(post.coverImage || post.title) && (
          <div className="post-cover-image">
            <img 
              src={post.coverImage || getOGImage(post.title, post.category, post.date)} 
              alt={post.title}
              style={{ width: '100%', height: 'auto', maxHeight: '400px', objectFit: 'cover', borderRadius: '8px' }}
            />
            {!post.coverImage && (
              <p className="image-credit">Generated by Demystify</p>
            )}
            {post.coverImage && (
